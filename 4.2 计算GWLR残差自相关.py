...
#   X_binary = [1 if x > 0.5 else 0 for x in X]
    ...
    # 计算模型的残差
    residuals = y_local - prediction_prob
    # 计算残差的平均值
    average_residual = np.nanmean(residuals)
    ...
    return result, count_same, pseudo_r_squared, aicc,average_residual
...
for i in unique_indices:
    ...
    local_result, count, pseudo_r_squared, aicc, average_residul = local_logistic_regression_final(target_point, X, Y, coordinates, best_global_bandwidth, X_local, Y_local)
    final_result.append(local_result)
    city_residual.append(average_residul)
...
# 假设df是原始数据集
coordinates = df.loc[unique_indices, [longitude_var, latitude_var]].values
residuals = city_residual


# 创建一个包含坐标和残差的DataFrame
residuals_df = pd.DataFrame(coordinates, columns=['当地经度', '当地纬度'])
residuals_df['residuals'] = residuals


# 创建GeoDataFrame
geometry = [Point(xy) for xy in zip(residuals_df['当地经度'], residuals_df['当地纬度'])]
geo_data = gpd.GeoDataFrame(residuals_df, geometry=geometry)


# 创建空间权重矩阵
w = DistanceBand.from_dataframe(geo_data, threshold=1.2, binary=False, silence_warnings=True)
